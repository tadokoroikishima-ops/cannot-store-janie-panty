...
for (var arm = 0; arm < numArms; arm++) {
  ...
  for (var li = 0; li < linesPerArm; li++) {
    ...
    /* 外側ほど細く・透明に */
    var alpha = (0.12 + progress * 0.07) * (0.7 + Math.sin(t * 0.5 + seed) * 0.3);
    var lw    = 0.5 + (1 - progress) * 1.2;
...
for (var j = 0; j < pts.length - 1; j++) {
  ...
  var alpha2 = (0.18 + taper * 0.16) * (0.8 + Math.sin(ph + j * 0.4) * 0.2);
